...
router.post('/register', async(req, res) => {
  ...
  const { identification, name,lastname, email, password, phone, birthday, gender, province, canton, distrit } = req.body;
...
bcrypt.hash(password, 10)
  .then(hash => {
      User.create({ identification, name,lastname, email, password: hash, phone, birthday, gender, province, canton, distrit })
          .then(users => res.json(users))
          .catch(err => res.json(err))
  }).catch(err => console.log(err.message))

if(!!!identification || !!!name || !!!lastname || !!!email || !!!password || !!!phone || !!!birthday || !!!gender || !!!province || !!!canton || !!!distrit){
