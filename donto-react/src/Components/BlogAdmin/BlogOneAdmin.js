...
class BlogOneAdmin extends Component {
    render() {
        let BlogDataAdmin = this.props.BlogDataAdmin;

        return (
            <section className="blog-wrapper blog-one section-padding pt-0">
                <div className="container">
                    
                    <SectionTitleOne BigTitle="Blogs Recientes" />

                    <div className="row">
                        {
                            BlogDataAdmin.postItem.map( (item, index) => {
                                return (
                                    index < 3 ?
